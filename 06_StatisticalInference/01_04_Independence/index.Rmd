...
- $A = \{\mbox{Head on flip 1}\}$ ~ $P(A) = .5$   ("~" means "i.e.")
...
*_If_ a collection of random variables $X_1, X_2, \ldots, X_n$ are _independent_, then their joint distribution is the product of their individual densities or mass functions*
...
- that means:  
  $$
  p(1, 0, 0, 1) = p(1, 1, 0, 0) = p(0, 0, 1, 1)
  $$ 
  because it depends only on the sum of successes (ones)
...
- The following are useful facts about covariance
  ...
  3. $|Cov(X, Y)| \leq \sqrt{Var(X) Var(y)} = sd(X) * sd(Y)$
...
$$
  ...
  2. $Cor(X, Y) = \pm 1$ if and only if $X = a + bY$ for some constants $a$ and $b$
     i.e. perfect correlation means linear relationship between $X$ and $Y$
...
This is the _population_ correlation, not the sample correlation (the sample correlation is an estimate of the population correlation).
---
...
That is, the variance of the mean is $\frac{\sigma^2}{n}$ ($\sigma is sd).  

---

## Some comments

- When $X_i$ are independent with a common variance, then $Var(\bar X) = \frac{\sigma^2}{n}$
...
- Standard deviation of a single die roll: $\sigma$  
- Standard deviation of 10 die rolls: $\sigma/\sqrt{n}$: "the standard deviation of the _distribution_ of the sample mean"
...
$var(x)$: the variance of the _this particular_ distribution (in the histogram): how variable are sons' heights
$var(x)/n$: how variable are _averages_ of sons' heights! To find out this variance, we'd need to have many samples of sons' heights and calculate
their averages. But (under the assumption of iid) we don't need this, because we can estimate the this variance with $var(x)/n$.
